...
# with open(results_path + f'_policy_modes_{str_time}', 'wb') as f:
#         pickle.dump(do.agent_policy_modes[:1] + do.agent_policy_modes[2:], f)

# TODO implement seed != 0 example!
def get_relevant_strategies(poaching_timestr, logging_timestr, size):
    ...
    agent_policy_modes = ['poaching'] * len(strategies_P) + ['logging'] * len(strategies_L)
    ...
    return all_agent_strategies, agent_policy_modes, eq_P, eq_L
...
def get_agent_poaching_logging_eq(all_agent_strategies, agent_policy_modes, agent_eq):
    ...
    agent_eq_logging_mask = 1 - agent_eq_poaching_mask
    agent_eq_array = np.array(agent_eq)
    agent_eq_poaching = agent_eq_array * agent_eq_poaching_mask
    agent_eq_poaching = agent_eq_poaching / agent_eq_poaching.sum()
    agent_eq_logging = agent_eq_array * agent_eq_logging_mask
    agent_eq_logging = agent_eq_logging / agent_eq_logging.sum()
    return agent_eq_poaching, agent_eq_logging
...
if __name__ == '__main__':
    ...
    agent_strategies, agent_policy_modes, eq_P, eq_L = get_relevant_strategies(poaching_timestr, logging_timestr, size)
